...
In order to successfully and safely integrate this sytem  with other systems it needs to be directly linked to another webpage. This can be achieved by the use of a portal window that runs a script which can fetch and display the page hosting this system directly. Recommended modules to use are:
 \begin{itemize}
	\item[$\bullet$]AJAX
	\item[$\bullet$]JSON
	\\
\end{itemize}
On integration, the sytem needs only to offer its core services in order to minimize faults and due to platform compability. However these functionalities should be kept in mind:
\begin{itemize}
	\item[$\bullet$]Low resource consumption(Mobile use)
	\item[$\bullet$]Good performance
	\item[$\bullet$]Reliable integration
	\item[$\bullet$]Safety during use as an integrated service
	\\
\end{itemize}
